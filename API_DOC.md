...
- **细粒度改写**: 默认 `AIGC_REWRITER_SENTENCES_PER_CALL=2`，长段会按句切分，每次模型调用只处理两句，再拼回完整段落
...
11. `AIGC_REWRITER_SENTENCES_PER_CALL=2` 会将长段拆成每两句一组分别改写，降低长段扩写、漏改和引用占位符丢失风险；设为更大的数可减少模型调用次数，设为 `1` 可逐句处理
12. 服务会跳过参考文献、图题、表题、章节标题、关键词等结构化文本，避免破坏格式
